// ================================================================
// uPLC v6 Junior — Sketch generado automáticamente
// ETI Patagonia - prof.martintorres@educ.ar
// ================================================================
// HARDWARE: Arduino NANO
// I1=D2 I2=D4 I3=D5 I4=D6 I5=D7 I6=D8 I7=D9
// Q1=D13 Q2=D14 Q3=D15 Q4=D16 Q5=D17 Q6=D3(PWM)
// A0=A6  A1=A7  | LCD I2C: A4(SDA) A5(SCL) addr 0x27
// Buzzer=D8(I7) | SoftSerial: RX=D10 TX=D11

#include <Wire.h>
#include <LiquidCrystal_I2C.h>

LiquidCrystal_I2C lcd(0x27, 20, 4);

// === Salidas digitales ===
bool Q1=false,Q2=false,Q3=false,Q4=false,Q5=false;
uint8_t Q6=0;

void playMelody(const char* mel){
  // Melodías predefinidas (fragmentos)
  int bday[]={262,262,294,262,349,330,0,-1};
  int mario[]={659,659,0,659,0,523,659,784,0,392,-1};
  int* notes = bday;
  if(strcmp(mel,"mario")==0) notes=mario;
  for(int i=0; notes[i]!=-1; i++){
    if(notes[i]==0) noTone(8); else tone(8,notes[i],180);
    delay(200); noTone(8); delay(40);
  }
}

void setup() {
  lcd.init();
  lcd.backlight();
  lcd.clear();
  // Entradas digitales
  pinMode(2,INPUT); pinMode(4,INPUT); pinMode(5,INPUT);
  pinMode(6,INPUT); pinMode(7,INPUT); pinMode(8,INPUT); pinMode(9,INPUT);
  // Salidas digitales
  pinMode(13,OUTPUT); pinMode(14,OUTPUT); pinMode(15,OUTPUT);
  pinMode(16,OUTPUT); pinMode(17,OUTPUT); pinMode(3,OUTPUT);
  // — Al iniciar —
    lcd.clear();
    lcd.setCursor(0,0); lcd.print("Mi 1er PIANO!");
    lcd.setCursor(0,1); lcd.print(" I1=DO  I2=RE  I3=MI");
    lcd.setCursor(0,2); lcd.print(" I4=FA  I5=SOL  I6=LA");
    lcd.setCursor(0,3); lcd.print(" I7= SI");
    tone(8, 440, 500); delay(500+5);
    delay(100);
    tone(8, 440, 500); delay(500+5);
    delay(100);
    tone(8, 349, 350); delay(350+5);
    delay(100);
    tone(8, 523, 150); delay(150+5);
    delay(100);
    tone(8, 440, 500); delay(500+5);
    delay(100);
}

void loop() {
  // === Leer entradas ===
  // (entradas leídas inline en cada bloque)
  // === Escribir salidas ===
  digitalWrite(13, Q1);
  digitalWrite(14, Q2);
  digitalWrite(15, Q3);
  digitalWrite(16, Q4);
  digitalWrite(17, Q5);
  analogWrite(3, Q6);
  // — Para siempre —
    if (digitalRead(2)) {
        tone(8, 262, 200); delay(200+5);
    }
    if (digitalRead(4)) {
        tone(8, 294, 200); delay(200+5);
    }
    if (digitalRead(5)) {
        tone(8, 330, 200); delay(200+5);
    }
    if (digitalRead(6)) {
        tone(8, 349, 200); delay(200+5);
    }
    if (digitalRead(7)) {
        tone(8, 392, 200); delay(200+5);
    }
    if (digitalRead(8)) {
        tone(8, 440, 200); delay(200+5);
    }
    if (digitalRead(9)) {
        tone(8, 494, 200); delay(200+5);
    }
  delay(10);
}
